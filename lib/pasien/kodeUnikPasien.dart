...
import 'package:test_bloc_firebase_1/ReadData/getDatabaseNdn.dart';
import 'package:test_bloc_firebase_1/scanning.dart';
import '../Reusable/LoginTombolNoBloc.dart';
import '../Reusable/textForm.dart';

class KodeUnikPasien extends StatefulWidget {
  const KodeUnikPasien({super.key});

  @override
  State<KodeUnikPasien> createState() => _KodeUnikPasienState();
}

class _KodeUnikPasienState extends State<KodeUnikPasien> {
  ...
  Future<void> getDataNdn() async {
    ...
    if (_isDataSnapshotNdn.docs.isNotEmpty) {
      ...
      if (myData['kodeUnik'] == KodeUnik.text.trim()) {
        Navigator.pushNamed(context, '/monitoringPasien');
      } else if (myData['kodeUnik'] == KodeUnik.text.trim().isEmpty) {
        Fluttertoast.showToast(
            msg: 'Mohon diisi kode uniknya',
